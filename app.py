...
COMPACT      = os.getenv("COMPACT", "0").strip() == "1"      # modo compacto para embed
...
# --------- STYLE (profissional + modo compacto) ---------
base_css = """
<style>
:root{
  --radius:16px; --card:#0c111a; --border:#233049; --muted:#8ea2c0;
...
.qa-q{
  background: var(--qbg); color:var(--qtxt);
  padding: VAR_QPAD; border-radius:12px; font-weight:600; margin-bottom:12px;
}
.qa-a{ color: var(--atxt); font-size: VAR_FSZ; line-height: VAR_LH; }
.qa-time{ color:var(--muted); font-size:VAR_TSZ; margin-left:auto; }
.ask-card{
  background:#0b0f16; border:1px solid var(--border); border-radius:14px;
...
def ai_summary_paragraph(question: str, df: pd.DataFrame, sql_used: str) -> str:
    """Texto profissional em 1–2 parágrafos curtos; nada de bullets; sem suposição."""
...
with c1:
    question_text = st.text_area("Faça sua pergunta:", height=84,
                                 placeholder="Ex.: Performance orgânica de agosto de 2025 para mobile no Brasil")
with c2:
...
# Ao enviar: cria a thread (ainda sem resposta) e agenda processamento
...
# Se existir thread pendente, processa UMA vez e salva
if st.session_state.pending_index is not None:
    try:
        ...
        if not df.empty:
            ...
            th["df_sample"] = df.head(300).to_dict(orient="records")  # evita requery no render
