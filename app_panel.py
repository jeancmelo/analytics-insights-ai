...
def sql_is_safe(sql: str, allowed_table_fqn: str) -> bool:
    ...
    # garante que a query referencia a view/tabela permitida
...
def validate_sql_columns(sql: str, schema_cols: list) -> list:
    """Retorna lista de colunas inválidas (inventadas) presentes no SQL."""
    valid = {c for c, _ in schema_cols}

    # pega tokens (identificadores) do SQL
    tokens = set(re.findall(r"\b[a-zA-Z_][a-zA-Z0-9_]*\b", sql))

    # checa apenas campos que costumam aparecer no nosso modelo (reduz falsos positivos)
    watched_prefixes = ("ga_", "gsc_", "sf_", "url", "data_date", "crawl_date")
    suspects = {t for t in tokens if t.startswith(watched_prefixes)}

    # remove palavras reservadas comuns que podem aparecer como tokens
    reserved = {
        "select",
        "from",
        "where",
    ...
    suspects = {t for t in suspects if t.lower() not in reserved}

    invalid = sorted([t for t in suspects if t not in valid])
    return invalid
...
def _route_dataset_heuristic(question: str) -> str:
    ...
    if has_why and (has_gsc or has_ga4):
        return "AI_READY"
    if has_gsc and has_ga4:
        return "AI_READY"
    if has_gsc and not has_ga4:
        return "FACT_GSC"
    if has_ga4 and not has_gsc:
        return "FACT_GA4"
    return "AI_READY"
...
def build_sql_with_ai(question: str, table_fqn: str, columns: list, table_kind: str) -> str:
    ...
    system = (
        ...
        "Se não existir coluna no schema, NÃO use (não invente)."
    )

    if table_kind == "AI_READY":
        rules = (
            "- Se a pergunta não trouxer período, filtre os últimos 90 dias usando a coluna `data_date`.\n"
            "- Identificadores: use `url` (e, se precisar de chave estável, use `url_key`).\n"
            "- Métricas GA4: pageviews=SUM(ga_pageviews), sessions=SUM(ga_sessions), users=SUM(ga_users).\n"
            "- Métricas GSC: clicks=SUM(gsc_clicks), impressions=SUM(gsc_impressions), ctr=SAFE_DIVIDE(SUM(gsc_clicks), SUM(gsc_impressions)).\n"
            "- Posição (se agregares vários dias/URLs): use média ponderada por impressões: SAFE_DIVIDE(SUM(gsc_avg_position * gsc_impressions), SUM(gsc_impressions)).\n"
            "- Screaming Frog (sf_*): usa como evidência para explicar causas (indexability, status_code, word_count, title_length, response_time, canonical, meta_robots, crawl_depth).\n"
            "- Para rankings, ordene por impressions, clicks, sessions ou pageviews e limite resultados longos.\n"
        )
    elif table_kind == "FACT_GSC":
        rules = (
            "- Se a pergunta não trouxer período, filtre os últimos 90 dias usando a coluna `data_date`.\n"
            "- Identificador: use `url` (ou `url_key` se existir).\n"
            "- Métricas: clicks=SUM(gsc_clicks), impressions=SUM(gsc_impressions), ctr=SAFE_DIVIDE(SUM(gsc_clicks), SUM(gsc_impressions)).\n"
            "- Posição (se agregares): SAFE_DIVIDE(SUM(gsc_avg_position * gsc_impressions), SUM(gsc_impressions)).\n"
        ...
        rules = (
            "- Se a pergunta não trouxer período, filtre os últimos 90 dias usando a coluna `data_date`.\n"
            "- Identificador: use `url` (ou `url_key` se existir).\n"
            "- Métricas: pageviews=SUM(ga_pageviews), sessions=SUM(ga_sessions), users=SUM(ga_users).\n"
    ...
    user = (
        ...
        "- Comece diretamente com SELECT.\n"
        "- NÃO use colunas fora do schema listado acima.\n\n"
...
if send and q and q.strip():
    st.session_state.insights.insert(
        0, {"q": q.strip(), "findings": None, "ts": time.time(), "sql": None, "route": None, "table": None}
    )
    ...
    try:
        ...
        if current_source.startswith("Auto (Router)") or current_source.startswith("Rubis Gas"):
            ...
            if not sql or not sql_is_safe(sql, active_table):
                ...
                st.session_state.insights[idx]["table"] = active_table
            else:
                invalid_cols = validate_sql_columns(sql, schema_cols)
                if invalid_cols:
                    raise RuntimeError("SQL inválida (colunas inexistentes usadas): " + ", ".join(invalid_cols))
        ...
        # ---------------- Instagram ----------------
            ...
            st.session_state.insights[idx]["table"] = "Supermetrics"
        ...
        # ---------------- Facebook ----------------
            ...
            st.session_state.insights[idx]["table"] = None
...
if st.session_state.insights:
    ...
    table_used = block.get("table")

    if route_used or table_used:
        parts = []
        if route_used:
            parts.append(f"View usada: {escape(str(route_used))}")
        if table_used:
            parts.append(f"Tabela/VIEW: {escape(str(table_used))}")
        st.markdown(
            f"<div style='color:#6b7280;font-size:12px;margin-bottom:8px;'>{' | '.join(parts)}</div>",
            unsafe_allow_html=True,
        )
