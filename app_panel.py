...
def build_sql_with_ai(question: str, table_fqn: str, columns: list) -> str:
    ...
    user = (
        ...
        f"- Métricas GSC:\n"
        f"  - clicks = SUM(gsc_clicks)\n"
        f"  - impressions = SUM(gsc_impressions)\n"
        f"  - ctr = SAFE_DIVIDE(SUM(gsc_clicks), SUM(gsc_impressions))\n"
        f"  - avg_position = AVG(gsc_avg_position)\n"
        f"- Métricas GA4:\n"
        f"  - pageviews = SUM(ga_pageviews)\n"
        f"  - sessions = SUM(ga_sessions)\n"
        f"- Para análise temporal, pode usar deltas (delta_*_7d).\n"
        f"- Para diagnóstico, pode usar flags (flag_*).\n"
        f"- Para rankings, ordene por impressions, clicks ou sessions.\n"
